      ...
      <div className="mx-auto w-full max-w-3xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Real-time updates for <span className="font-semibold text-slate-700">{listing.propertyName}</span>. Link
            valid until {ownerToken.expiresAt.toLocaleString("en-MY", { dateStyle: "medium", timeStyle: "short" })}.
          </p>
        </div>
        <Card>
          ...
          <div className="mt-6 space-y-6">
            {steps.map((stage, index) => {
              ...
              return (
                <div key={stage} className="grid grid-cols-[auto,1fr] gap-4">
                  <div className="relative flex flex-col items-center">
                    ...
                    >
                      <span
                        className={`h-2 w-2 rounded-full ${isCompleted ? "bg-primary" : "bg-slate-300 dark:bg-slate-600"}`}
                      />
                    </span>
                    {!isLast ? (
                      <span
                        className="absolute top-6 bottom-[-1.5rem] w-px bg-slate-200 dark:bg-slate-800"
                        aria-hidden="true"
                      />
                    ) : null}
                  </div>
                    ...
                    {log?.viewings?.length ? (
                      <ul className="mt-3 space-y-1 text-xs text-slate-600">
                        {log.viewings.map((viewing) => (
                          <li
                            key={viewing.id}
                            className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-2 first:border-t-0 first:pt-0 dark:border-slate-700"
                          >
                            <span className="font-medium text-slate-800 dark:text-slate-100">{viewing.name}</span>
